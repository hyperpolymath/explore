A batch file is a script file in DOS, OS/2 and Microsoft Windows. It
...
`+IF+`, `+FOR+`, and `+GOTO+` labels. The term "`batch`" is from batch
processing, meaning "`non-interactive execution`", though a batch file
might not process a batch of multiple data.
